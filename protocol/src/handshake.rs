...
impl<'a> Handshake<SentKey<'a>> {
    ...
    pub fn receive_key(
        ...
        if Magic::from_params(self.network).unwrap()
...
mod tests {
    ...
    fn test_v1_protocol_detection() {
        ...
        let magic = Magic::from_params(Network::Bitcoin).unwrap().to_bytes();
        ...
        let magic = Magic::from_params(Network::Signet).unwrap().to_bytes();
        v1_testnet_key[..4].copy_from_slice(&magic);
